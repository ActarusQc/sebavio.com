...
export function FeatureGrid() {
  ...
  return (
    <section
      id="fonctionnalites"
      className="scroll-mt-24 bg-white py-[3.5rem] sm:py-16 lg:py-[4.25rem]"
    ...
    >
      <div className="mx-auto max-w-[96rem] px-[clamp(1.5rem,4vw,4.5rem)]">
        ...
        <div className="mt-10 grid gap-8 sm:grid-cols-2 sm:gap-6 xl:grid-cols-4 xl:gap-0">
          {features.items.map((item, index) => (
            <FadeIn key={item.id} delay={0.03 * index}>
              <article
                className={cn(
                  "flex h-full flex-col xl:px-6",
              ...
              >
                ...
                <h3 className="font-heading text-[1.2rem] leading-snug font-semibold text-[#082b46] sm:text-[1.35rem]">
                  {item.title}
                </h3>
                <p className="mt-2 text-[1rem] leading-relaxed text-[#60758a]">
                  {item.body}
                </p>
              </article>
            </FadeIn>
